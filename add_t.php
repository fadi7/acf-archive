...
$des=$_POST["desc"];
$date=$_POST["date"];
$s="update box set b_id=$bid ,`desc`='$des' , `date`='$date' where `id`=$id";
...
foreach ($_FILES["files"]["error"] as $key => $error) {
   ...
   $e = strtolower(array_pop($value));
		...
		$s="SELECT * FROM `box` WHERE year='{$_SESSION['year']}' and type='2';";
...
$b.="لا يوجد مرفقات";
$b.=<<<ab
<th style="width:2%"><a href="?page=add_t&edit=ok&id=$res[0]" ><i class='fa fa-edit' aria-hidden='true' style='font-size:20px'></a></th>
<th style="width:2%" ><a  onclick="javascript:confirmDelete('?page=add_t&d=$res[0]')"><i class='fa fa-trash-o' aria-hidden='true' style='font-size:20px'></a></th>	
...
{
             AddContent("اضافة تعميم جديد",'title',0);
             AddContent("اضافة تعميم جديد",'title2',0);
$sql=mysqli_query($con,"select max(b_id)from box where type='2' and year = '{$_SESSION['year']}' ;");
...
<div class="col-md-12">
      <div class="card card-mini">
        <div class="card-header">
          <div class="card-title"> اضافة تعميم    <strong class="highligh">&nbsp;&nbsp; جديد &nbsp;&nbsp;</strong></div>
        </div>
        <div class="card-body">
		 <div class="section zz" style="">
                  <div class="section-body __indent ">
        <form id="submit_this" enctype="multipart/form-data" method="post"  action="?page=add_t" class="form-horizontal eform">
...
<input type="file" class="form-control" name='files[]' multiple="" id="filesToUpload"  onChange="makeFileList();">
  ...
  <input type='hidden' value="موافق" name='insert'>  <button type="submit" class="btn  btn-success "  id="submita" >موافق</button>
            <button type="button" class="btn  btn-default" data-dismiss="modal" onclick="javascript:window.location='?page=add_t'">إلغاء</button>&nbsp; 
</form>				
              </div></div>
     </div>
      </div>
...
unlink("box/{$file}.{$ex}");
    ...
    $id=addslashes($_GET["id"]);
       AddContent('',"links2",0);
             AddContent("تعديل تعميم",'title',0);
             AddContent("تعديل تعميم",'title2',0);
...
<div class="col-md-12">
      <div class="card card-mini">
        <div class="card-header">
          <div class="card-title"> تعديل التعميم رقم   <strong class="highligh">&nbsp;&nbsp; $bid &nbsp;&nbsp;</strong></div>
...
<label class="col-md-2 control-label">الملف الرئيسي :</label>
      ...
      <a  class="list-group-item f">
        <span class="point" onclick="window.open('box/$fid.$e', '_blank')"> الملف الرئيسي </span><span onclick="confirmDelete('?page=add_t&fd=$fid&edit=ok&id=$id&e=$e')" class="badge" style="float:left;">
...
$a.=<<<a
      ...
      <a  class="list-group-item f">
        <span class="point" onclick="window.open('box/{$res[0]}.$e', '_blank')"> المرفق $n </span><span onclick="confirmDelete('?page=add_t&fd={$res[0]}&edit=ok&id=$id&e=$e')" class="badge" style="float:left;">
		<span class="point glyphicon glyphicon-trash"></span></span>
      </a>
a;
$n++;
